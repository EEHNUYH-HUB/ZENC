<template>
  <div class="toast fade show p-2 mt-2" :class="getColor(color)">
    <div class="toast-header bg-transparent border-0">
      ...
      <span class="me-auto font-weight-bold" @click="closeHandler(data,true)" :class="getTextColor(color)">
        {{ title }}
      </span>
      <small :class="getTextColor(color)">{{ date }}</small>
      <i class="fas fa-times text-md ms-3 cursor-pointer" :class="getTextColor(color)"
        @click="closeHandler(data,false)" />
    </div>
    <hr class="horizontal dark m-0" :class="getHrColor(color)" />
    <div class="toast-body" @click="closeHandler(data,true)" :class="getTextColor(color)">{{ description }}</div>
  </div>
</template>
